...
1. 把 `skill.zip` 直接上传或拖给支持文件与终端操作的 AI Agent，例如 Codex、Hermes 或 Claude Code；
2. 告诉 Agent：**“帮我安装这个 Skill，并按照 README 配置 FlowGate API Key。”**
3. Agent 自动解压、安装、创建配置文件并运行测试；
4. 以后只需粘贴一条链接，就能开始给知识银行“存钱”。
...
- **推荐：交给 AI Agent 自动安装。** 适合绝大多数用户，不需要理解目录和命令；
- **备用：自己手动安装。** 适合希望完全控制安装位置的用户。

整个过程通常只需要 5 分钟。

---

## 方法一：把安装包直接丢给 AI Agent（最推荐）

只要你的 AI Agent 能读取附件、解压文件、编辑本地文件并运行终端命令，就可以让它代替你完成安装。
...
- Hermes；
- Claude Code；
- 其他具有本地文件和终端操作能力的 AI Agent。

### 第 1 步：把 `skill.zip` 上传给 Agent

把下载好的完整安装包直接拖进对话，或者放进 Agent 当前可以访问的工作目录。

> 不要只上传 `SKILL.md`。请上传完整的 `skill.zip`，因为脚本、配置模板和 API 文档都在压缩包中。

### 第 2 步：复制下面这段安装指令

```text
请帮我安装附件中的 FlowGate Video Parser Skill。

要求：
1. 解压安装包并检查其中的 SKILL.md、README.md、scripts 和 references；
2. 根据你当前运行环境，安装到可长期使用的个人 Skill 目录；如果当前环境不支持全局安装，则安装到当前项目的 Skill 目录；
3. 不要遗漏压缩包内的任何必要文件；
4. 将 .env.example 复制为 .env.local；
5. 提示我提供 FlowGate API Key，并只把它写入本地 .env.local 的 FLOWGATE_API_KEY；
6. 不要在回复、日志、README、代码或 Git 提交中回显我的完整 API Key；
7. 确保 .env.local 已加入 .gitignore，不要提交到仓库；
8. 使用内置 flowgate.py 的 balance 命令验证配置；
9. 验证成功后，告诉我实际安装路径，并给出一条首次解析视频的示例指令；
10. 如果遇到问题，请直接检查并修复，不要只给我泛泛的操作建议。
```

你也可以使用更短的一句话：

```text
安装这个 FlowGate Skill，按 README 帮我配置 API Key，并运行 balance 验证；密钥只能保存在本地 .env.local，禁止回显或提交 Git。
```

### 第 3 步：按照 Agent 提示配置 API Key

Agent 解压并检查项目后，通常会提示你提供 FlowGate API Key。

先打开 [FlowGate 登录页](https://kb.apimar.online/login)，注册或登录账号，在工作台复制自己的 API Key。

有两种配置方式：

#### 方式 A：让可信的本地 Agent 直接写入

在确认 Agent 运行于你信任的本地环境后，把 API Key 发给它，并明确要求：

```text
请将这个 Key 只写入 FlowGate Skill 目录下的 .env.local，
不要在回复中重复它，也不要写入代码、README 或 Git：

你的 API Key
```

#### 方式 B：Agent 创建文件，你自己粘贴（更安全）

你也可以不把完整 Key 发给 Agent，让它先创建：

```env
FLOWGATE_API_KEY="请在这里粘贴你的 Key"
FLOWGATE_BASE_URL="https://kb.apimar.online"
FLOWGATE_OUTPUT_DIR="./reports"
```

然后你在本地编辑 `.env.local`，把占位文字替换成真实 API Key。

> 在共享会话、远程公共 Agent、直播、录屏或团队可见环境中，建议使用方式 B。
...
```text
现在运行 balance 命令验证 FlowGate 配置。如果失败，请检查安装路径、.env.local 文件名、Python 命令和 API Key 读取逻辑，并直接修复。
```

验证成功后，再发送一条真实链接：

```text
调用刚安装的 FlowGate Skill，解析这个链接并保存成 Markdown：
https://你的公开内容链接
```

看到 Markdown 文件成功生成，就代表安装完成。
...
- 已找到并读取 `SKILL.md`；
- Skill 已安装到个人目录或当前项目目录；
- `.env.local` 已创建；
- API Key 没有被公开回显；
- `balance` 命令验证成功；
- 给出了实际安装路径；
- 能成功解析一条链接并输出 Markdown。
...
安装成功后，你不需要记住任何命令。直接把链接和目标告诉 Agent 即可。

例如：

```text
调用 FlowGate，把这个短视频转成 Markdown，并保存到我的知识库 Inbox：
https://内容链接
```

```text
先用 FlowGate 解析这篇图文，再整理成：核心观点、关键案例、可执行动作和原始来源。
```

```text
把表格中的全部链接批量解析，成功结果打包，失败项单独列出原因。
```

Agent 应当自动读取 Skill 的 `SKILL.md`，调用内置脚本，并根据你的目标组织最终结果。
